...
In this section we go over the $\PSPACE$-completeness result of the SLIDING TOKENS problem, proved by a reduction from NCL. As seen in
...
\begin{theorem}SLIDING TOKENS problem is $\PSPACE$-complete. \end{theorem}\label{theorem:ncl_sliding_token}
\begin{proof}
...
\begin{example}{C2E to SLIDING TOKENS problem reduction. \\}
...
\begin{figure}[H]\ContinuedFloat
  ...
  \begin{subfigure}[b]{0.6\textwidth}
    ...
    \label{fig:output_instance_final}
  \end{subfigure}
\caption{Reconfiguration sequence which transforms the initial configuration $S_0$ of the corresponding output SLIDING TOKENS instance to the target configuration $S_3$.}
\label{fig:input_instance_config_to_edge}
\end{figure}
\end{example}


\section{LABELLED variant of SLIDING-TOKENS Problem}\label{sec:labelled_sliding_token}

The LABELLED SLIDING TOKENS problem is a variant of the SLIDING TOKENS problem where each token has a unique label. The purpose of this section is
to prove that the SLIDING TOKENS problem remains $\PSPACE$-hard even with labelled tokens. We will first start by giving a formal definition of the
LABELLED SLIDING TOKENS problem.

\begin{defn}{(LABELLED SLIDING TOKENS problem)}
  Suppose that we are given two independent sets $I_b$ and $I_r$ of a graph $G = (V,E)$ such that $|I_b| = |I_r|$ and imagine that a
...
\begin{example}{C2E to LABELLED SLIDING TOKENS problem reduction. \\}
  \textbf{Input instance : C2E for restricted NCL same as figure \ref{fig:input_instance_config_to_config}.} \hfill

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% Output instance %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%------------------------------------------- out 1 & 2 ---------------------------------------------
  \textbf{Output instance : LABELLED SLIDING TOKENS instance.} \hfill
    ...
    \begin{subfigure}[b]{0.6\textwidth}
      \begin{scaletikzpicturetowidth}{\textwidth}
        \begin{tikzpicture}[scale=0.8]
          ...
          % Tokens
          \path[fill] (\xa-0.5,\ya-0.8) circle (\ver);   %v3
          \node (a) at (\xa-1.4,\ya-0.8) {\textcolor{violet}{\large $t_1$}};
          %labels
          \draw[thick] (\xa,\ya)--(\xa-0.5,\ya-0.8);
          \draw[thick] (\xa,\ya)--(\xa+0.5,\ya-0.8);
          \path[draw, thick, dashed, rounded corners=4mm] (\xa,\ya+0.6)--(\xa+1.2,\ya-1.2)--(\xa-1.2,\ya-1.2)--cycle;
          ...
          % Tokens
          \path[fill] (\xb+0.5,\yb-0.8) circle (\ver);   %v3
          \node (b) at (\xb+1.4,\yb-0.8) {\textcolor{violet}{\large $t_3$}};
          %labels
          \draw[thick] (\xb,\yb)--(\xb-0.5,\yb-0.8);
          \draw[thick] (\xb,\yb)--(\xb+0.5,\yb-0.8);
          \path[draw, thick, dashed, rounded corners=4mm] (\xb,\yb+0.6)--(\xb+1.2,\yb-1.2)--(\xb-1.2,\yb-1.2)--cycle;
          ...
          % Tokens
          \path[fill] (\xc-0.5,\yc-0.8) circle (\ver);   %v3
          \node (c) at (\xc-1.4,\yc-0.8) {\textcolor{violet}{\large $t_2$}};
          %labels
          \draw[thick] (\xc,\yc)--(\xc-0.5,\yc-0.8);
          \draw[thick] (\xc,\yc)--(\xc+0.5,\yc-0.8);
          \path[draw, thick, dashed, rounded corners=4mm] (\xc,\yc+0.6)--(\xc+1.2,\yc-1.2)--(\xc-1.2,\yc-1.2)--cycle;
          ...
          \path[fill] (\xd-1.5,\yd-1.5) circle (\ver);   %v5
          \path[fill] (\xd+1.5,\yd-1.5) circle (\ver);   %v6
          \node (d) at (\xd+0.8,\yd) {\textcolor{violet}{\large $t_7$}};
          \node (e) at (\xd+2.4,\yd-1.5) {\textcolor{violet}{\large $t_8$}};
          \node (f) at (\xd-2.4,\yd-1.5) {\textcolor{violet}{\large $t_9$}};
          ...
          \path[fill] (\xe+0.5,\ye-0.8) circle (\ver);   %v4
          \path[fill] (\xe-1.5,\ye-1.5) circle (\ver);   %v5
          \node (d) at (\xe-0.8,\ye+1) {\textcolor{violet}{\large $t_4$}};
          \node (e) at (\xe+1.4,\ye-0.8) {\textcolor{violet}{\large $t_5$}};
          \node (f) at (\xe-2.4,\ye-1.5) {\textcolor{violet}{\large $t_6$}};
          %labels
          \draw[thick] (\xe,\ye+1)--(\xe, \ye)--(\xe-0.5,\ye-0.8)--(\xe-1.5,\ye-1.5);
          \draw[thick] (\xe-0.5,\ye-0.8)--(\xe+0.5,\ye-0.8)--(\xe+1.5,\ye-1.5);
          \draw[thick] (\xe,\ye)-- (\xe+0.5,\ye-0.8);
          ...
          \path[fill] (\xf-0.5,\yf-0.8) circle (\ver);   %v3
          \path[fill] (\xf+1.5,\yf-1.5) circle (\ver);   %v6
          \node (g) at (\xf+0.8,\yf+1){\textcolor{violet}{\large $t_{10}$}};
          \node (h) at (\xf-1.4,\yf-0.8) {\textcolor{violet}{\large $t_{11}$}};
          \node (i) at (\xf+2.4,\yf-1.5) {\textcolor{violet}{\large $t_{12}$}};
    ...
    \begin{subfigure}[b]{0.6\textwidth}
      \begin{scaletikzpicturetowidth}{\textwidth}
        \begin{tikzpicture}[scale=0.8]
          ...
          \path[fill] (\xa+0.5,\ya-0.8) circle (\ver);   %v4
          \node (a) at (\xa-1.4,\ya-0.8) {\textcolor{violet}{\large $t_1$}};
          \node (a) at (\xa+1.4,\ya-0.8) {\textcolor{violet}{\large $t_2$}};
          %labels
          \draw[thick] (\xa,\ya)--(\xa-0.5,\ya-0.8);
          \draw[thick] (\xa,\ya)--(\xa+0.5,\ya-0.8);
          \path[draw, thick, dashed, rounded corners=4mm] (\xa,\ya+0.6)--(\xa+1.2,\ya-1.2)--(\xa-1.2,\ya-1.2)--cycle;
  ...
  \begin{subfigure}[b]{0.6\textwidth}
    \begin{scaletikzpicturetowidth}{\textwidth}
      \begin{tikzpicture}[scale=0.8]
        ...
        % Tokens
        \path[fill] (\xc+0.5,\yc-0.8) circle (\ver);   %v4
        \node (a) at (\xc+1.4,\yc-0.8) {\textcolor{violet}{\large $t_3$}};
        %labels
        \draw[thick] (\xc,\yc)--(\xc-0.5,\yc-0.8);
        \draw[thick] (\xc,\yc)--(\xc+0.5,\yc-0.8);
        \path[draw, thick, dashed, rounded corners=4mm] (\xc,\yc+0.6)--(\xc+1.2,\yc-1.2)--(\xc-1.2,\yc-1.2)--cycle;
  ...
  \begin{subfigure}[b]{0.6\textwidth}
    \begin{scaletikzpicturetowidth}{\textwidth}
      \begin{tikzpicture}[scale=0.8]
        ...
        % Tokens
        \path[fill] (\xb,\yb)  circle (\ver);   %v1
        \node (a) at (\xb+0.9,\yb) {\textcolor{violet}{\large $t_4$}};
        %labels
        \draw[thick] (\xb,\yb)--(\xb-0.5,\yb-0.8);
        \draw[thick] (\xb,\yb)--(\xb+0.5,\yb-0.8);
        \path[draw, thick, dashed, rounded corners=4mm] (\xb,\yb+0.6)--(\xb+1.2,\yb-1.2)--(\xb-1.2,\yb-1.2)--cycle;
    ...
    \label{fig:output_instance_final}
  \end{subfigure}
  \caption{Reconfiguration sequence which transforms the initial configuration $S_0$ of the corresponding output LABELLED SLIDING TOKENS instance to the target configuration $S_3$.}
  \label{fig:input_instance_config_to_edge}
  \end{figure}
\end{example}
