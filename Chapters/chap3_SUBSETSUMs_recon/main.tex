...
is the points of $Q$ that lies in the hyperplane. The red path represents the reconfiguration sequence between the feasible solutions.
  ...
  \begin{figure}[H]
        ...
        \begin{tikzpicture}[scale=1.2, x={({1}, {0})}]
          ...
          \end{axis}
        \end{tikzpicture}
      \caption{Geometric visualisation of a given instance of the $3$-move subset sum reconfiguration problem.}
      \label{fig:geo_2}
  \end{figure}
\end{example}
...
A set $\mathcal{S}$ is said to be sum-free if the equation $x + y = z$ has no solutions within $\mathcal{S}$.